...
export default (app: App) => {
  ...
  app.view('composer', async ({ ack, body, client, view }) => {
    ...
    const destinationConversation =
      destination_conversation_input.select_destination_conversation
        .selected_conversation
    const sourceConversation =
      source_conversation_input.select_source_conversation.selected_conversation
    ...
    const selectedMessageType =
      message_type_input.select_message_type.selected_option.value
    const selectedUser = user_input.select_user.selected_user

    try {
      // Get relevant info
      const [{ channel }, { user }] = await Promise.all([
        client.conversations.info({ channel: sourceConversation }) as Promise<
      ...
      const wolfMessage: MessageAttachment = {
        ...
        color: 'D0D0D0',
        fallback:
          utils.getFallbackText(ts.toString(), user.name, inputMessage) +
          signature,
        footer: `${
          selectedMessageType === 'message' ? 'Posted' : 'From a thread'
        } in #${channel.name}`,
        mrkdwn_in: ['text'],
      ...
      await client.chat.postMessage({
        ...
        attachments: [
          ...
          ...(config.enableDisclaimers ? [{ blocks: [Disclaimer()] }] : [])
        ]
      })
    } catch (e) {
